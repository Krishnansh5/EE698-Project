...
class Dataset(data.Dataset):
    def __init__(self, gt_file, config, mask_file=None):
        self.gt_image_files = self.load_file_list(gt_file,config)
        self.gt_image_files = self.gt_image_files[:10000]
        ...
        # read masks from files
            ...
            self.mask_image_files = self.load_file_list(mask_file,config)
    ...
    def load_file_list(self, flist, config):
        ...
        if isinstance(flist, str):
            if os.path.isdir(flist):
                ...
                # if config.isTrain:
                #     flist = flist[:config.train_data_size]
                # else:
                #     flist = flist[config.train_data_size:config.train_data_size+config.test_data_size]
                flist.sort()
                return flist
    ...
    def load_mask(self, index, img):
        ...
        if self.mask_type == "random_bbox":
            ...
            return torch.from_numpy(mask)
        elif self.mask_type == "stitch":
            mask = generate_stitch_mask(image_shape)
            return torch.from_numpy(mask)
        elif self.mask_type == "from_file":
...
def generate_stitch_mask(shape):
    w = shape[0] // 3
    x1 = shape[0] // 2 - w // 2
    x2 = shape[0] // 2 + w // 2
    mask = np.ones((shape[0], shape[0]), np.float32)
    mask[:, :x1] = 0.0
    mask[:, x2:] = 0.0
    return mask.reshape((1,) + mask.shape).astype(np.float32)
