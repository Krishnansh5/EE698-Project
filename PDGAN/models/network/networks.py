...
from .context_encoder import _netG,_netlocalD
...
def define_G(
    ...
    if model_name is "PDGAN":
        if cfg.prior_net == "PConv":
            Enc = Encoder(cfg.input_nc, cfg.ngf, norm_layer=norm)
            Dec = Decoder(cfg.output_nc, cfg.ngf, norm_layer=norm)
            PDGANNet = SPDNormGenerator(cfg)
            return (
                init_net(PDGANNet, init_type, init_gain, gpu_ids),
                init_net(Enc, init_type, init_gain, gpu_ids),
                init_net(Dec, init_type, init_gain, gpu_ids),
            )
        else:
            PDGANNet = SPDNormGenerator(cfg)
            return(
