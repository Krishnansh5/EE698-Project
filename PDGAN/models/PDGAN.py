...
from .network import networks, context_encoder
...
class PDGAN(BaseModel):
    def __init__(self, cfg):
        ...
        if cfg.prior_net == "PConv":
            ...
            self.netG = context_encoder._netG(cfg)
            self.netG.load_state_dict(torch.load(cfg.netG,map_location=lambda storage, location: storage)['state_dict'])
            self.netG = self.netG.cuda(self.gpu_ids[0])
                # self.netD.load_state_dict(torch.load(cfg.netD,map_location=lambda storage, location: storage)['state_dict'])
            # logger.info(
    ...
    def forward(self):
        ...
        if self.cfg.prior_net == "PConv":
            ...
            self.pre_image = self.netG(self.input_img)
