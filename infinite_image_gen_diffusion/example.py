...
def generate_image_grid(
    ...
    seed=0, gridw=2, gridh=1, device=torch.device('cuda'),
    ...
    class_labels = None
    print('ll', net.label_dim)
    if net.label_dim:
        class_labels = torch.eye(net.label_dim, device=device)[torch.randint(net.label_dim, size=[batch_size], device=device)]

    # Adjust noise levels based on what's supported by the network.
    #print("class", class_labels.shape)
    c = training.augment.AugmentPipe
    ...
    for i, (t_cur, t_next) in tqdm.tqdm(list(enumerate(zip(t_steps[:-1], t_steps[1:]))), unit='step'): # 0, ..., N-1
        ...
        denoised = net(x_hat, t_hat, class_labels = c).to(torch.float64)
        d_cur = (x_hat - denoised) / t_hat
        x_next = x_hat + (t_next - t_hat) * d_cur

        # Apply 2nd order correction.
        if i < num_steps - 1:
            denoised = net(x_next, t_next, class_labels=c).to(torch.float64)
...
def main():
    generate_image_grid('network-snapshot-000000 copy 2.pkl', 'imagenet-64x64.png', num_steps=40, S_churn=40, S_min=0.05, S_max=50, S_noise=1.003)
